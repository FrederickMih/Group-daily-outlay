module GroupsHelper
  ...
  def delete_group_link(group)
     return unless group.user_id == current_user.id
    link_to '<i class="small material-icons deep-orange-text- accent-3">delete</i>'.html_safe, group, method: :delete, date: {confirm: 'Are you sure?'}
  end
  ...
  def delete_outlay_link(outlay)
     return unless outlay.user_id == current_user.id
    link_to '<i class="small material-icons deep-orange-text- accent-3">delete</i>'.html_safe, outlay, method: :delete, date: {confirm: 'Are you sure?'}
  end
